...
const dict = {
  ...
  'settings.keyboardNavigation': { ja: 'キーボードショートカット', en: 'Keyboard shortcuts' },
  'settings.keyboardNavigationDesc': { ja: 'FreshRSS 互換の単独キーショートカットを有効にします。オフにすると単独キーショートカットをすべて無効化します。', en: 'Enable FreshRSS-style single-key shortcuts. Turn this off to disable all single-key shortcuts.' },
  ...
  'settings.keybindingsDesc': { ja: 'FreshRSS スタイルの各アクションに割り当てるキーを変更できます', en: 'Customize the key assigned to each FreshRSS-style action' },
  'settings.keybindingsNext': { ja: '次の記事', en: 'Next article' },
  'settings.keybindingsNextUnread': { ja: '次の未読', en: 'Next unread article' },
  'settings.keybindingsPrev': { ja: '前の記事', en: 'Previous article' },
  'settings.keybindingsSkipNext': { ja: '次へスキップ', en: 'Skip to next article' },
  'settings.keybindingsSkipPrev': { ja: '前へスキップ', en: 'Skip to previous article' },
  'settings.keybindingsFirst': { ja: '先頭の記事', en: 'First article' },
  'settings.keybindingsLast': { ja: '末尾の記事', en: 'Last article' },
  'settings.keybindingsMarkRead': { ja: '既読切り替え', en: 'Toggle read' },
  'settings.keybindingsLike': { ja: 'いいね', en: 'Liked' },
  'settings.keybindingsBookmark': { ja: '後で読む', en: 'Read Later' },
  'settings.keybindingsOpenExternal': { ja: '元記事を開く', en: 'Open original' },
  'settings.keybindingsToggleMedia': { ja: 'メディア再生/停止', en: 'Play/pause media' },
  'settings.keybindingsCaptureHint': { ja: '入力欄を選択して、割り当てたいキーを押してください。Space / Home / End も使えます。', en: 'Focus a field and press the key you want to assign. Space, Home, and End are supported.' },
  'settings.keybindingsDuplicate': { ja: '同じキーが複数のアクションに割り当てられています', en: 'Duplicate key assignment detected' },
  'settings.fullTextBlacklist': { ja: '全文取得ブラックリスト', en: 'Full-text blacklist' },
  'settings.fullTextBlacklistDesc': { ja: 'クローリング制限が厳しいホストでは全文取得を行わず、RSS 本文だけを使います。1 行に 1 ホストずつ入力してください。', en: 'Skip full-text crawling on hosts with strict anti-crawling policies and keep RSS content only. Enter one hostname per line.' },
  'settings.fullTextBlacklistReset': { ja: 'デフォルトに戻す', en: 'Reset to default' },
  'settings.fullTextBlacklistSaved': { ja: '保存しました', en: 'Saved' },
  'settings.fullTextBlacklistHint': { ja: 'サブドメインも自動的に一致します。空にするとブラックリストを無効化できます。', en: 'Subdomains are matched automatically. Clear the list to disable the blacklist.' },
  ...
  'integration.freshrss': { ja: 'FreshRSS 連携', en: 'FreshRSS Sync' },
  'integration.freshrssDesc': {
    ja: 'FreshRSS の Fever API からフィードを取り込み、Oksskolten 側の既読・保存状態を FreshRSS に反映します。',
    en: 'Import feeds from FreshRSS via the Fever API and push Oksskolten read/saved state back to FreshRSS.',
  },
  'integration.freshrssStatus': { ja: '接続状態', en: 'Connection status' },
  'integration.freshrssConfigured': { ja: '設定済み', en: 'Configured' },
  'integration.freshrssNotConfigured': { ja: '未設定', en: 'Not configured' },
  'integration.freshrssEndpoint': { ja: 'Fever API URL', en: 'Fever API URL' },
  'integration.freshrssEndpointDesc': {
    ja: '例: https://reader.example.com/api/fever.php またはローカル検証用の http://127.0.0.1:9080/api/fever.php',
    en: 'Example: https://reader.example.com/api/fever.php or http://127.0.0.1:9080/api/fever.php for local testing',
  },
  'integration.freshrssEndpointSaved': { ja: 'FreshRSS の URL を保存しました', en: 'Saved FreshRSS endpoint URL' },
  'integration.freshrssApiKey': { ja: 'Fever API キー', en: 'Fever API key' },
  'integration.freshrssApiKeyDesc': {
    ja: 'FreshRSS の `username:apiPassword` を MD5 化した 32 文字の Fever キーを入力してください。',
    en: 'Enter the 32-character Fever key, which is the MD5 hash of `username:apiPassword` in FreshRSS.',
  },
  'integration.freshrssApiKeyConfigured': { ja: 'API キーは設定済みです', en: 'API key is configured' },
  'integration.freshrssApiKeyMissing': { ja: 'API キーが未設定です', en: 'API key is not configured' },
  'integration.freshrssApiKeySaved': { ja: 'FreshRSS の API キーを保存しました', en: 'Saved FreshRSS API key' },
  'integration.freshrssApiKeyDeleted': { ja: 'FreshRSS の API キーを削除しました', en: 'Deleted FreshRSS API key' },
  'integration.freshrssSyncToggle': { ja: '同期を有効化', en: 'Enable sync' },
  'integration.freshrssSyncToggleDesc': {
    ja: '有効にすると定期的なフィード同期と既読・保存状態の自動反映を行います。',
    en: 'When enabled, Oksskolten periodically syncs feeds and automatically pushes read/saved state changes.',
  },
  'integration.freshrssSyncDisabled': { ja: 'FreshRSS 同期は無効です', en: 'FreshRSS sync is disabled' },
  'integration.freshrssVerify': { ja: '接続テスト', en: 'Verify connection' },
  'integration.freshrssVerifySuccess': { ja: 'FreshRSS への接続に成功しました', en: 'FreshRSS connection succeeded' },
  'integration.freshrssSyncNow': { ja: '今すぐ同期', en: 'Sync now' },
  'integration.freshrssSyncResult': {
    ja: 'フィード: 新規 ${createdFeeds} 件、更新 ${updatedFeeds} 件。カテゴリ: 新規 ${createdCategories} 件、更新 ${updatedCategories} 件。',
    en: 'Feeds: ${createdFeeds} created, ${updatedFeeds} updated. Categories: ${createdCategories} created, ${updatedCategories} updated.',
  },
  'integration.freshrssLastSync': { ja: '前回の同期: ${time}', en: 'Last sync: ${time}' },
  'integration.freshrssLastError': { ja: '直近のエラー: ${error}', en: 'Last error: ${error}' },
  ...
  'openai.apiUrl': { ja: 'OpenAI API URL', en: 'OpenAI API URL' },
  'openai.apiUrlDesc': { ja: '互換APIを使う場合はベース URL を設定します。空欄なら OpenAI の標準 URL を使います', en: 'Set a base URL when using an OpenAI-compatible API. Leave empty to use the default OpenAI URL' },
  'openai.apiUrlPlaceholder': { ja: 'https://api.openai.com/v1', en: 'https://api.openai.com/v1' },
  'openai.apiUrlSaved': { ja: 'OpenAI API URL を保存しました', en: 'OpenAI API URL saved' },
  'openai.testConnection': { ja: '接続テスト', en: 'Test Connection' },
  'openai.testing': { ja: 'テスト中...', en: 'Testing...' },
  'openai.connected': { ja: '接続成功', en: 'Connected' },
  'openai.connectionFailed': { ja: '接続失敗', en: 'Connection failed' },
